...
impl TranspositionTable {
    ...
    #[inline(always)]
    pub fn store(&self, hash: u64, depth: i32, score: i32, node_type: NodeType, best_move: Move) {
        let idx = (hash as usize) & self.mask;
        let bucket = &self.buckets[idx];
        ...
        for (i, entry) in bucket.entries.iter().enumerate() {
            ...
            let age_penalty = if entry_age != gen { -1000 } else { 0 };
...
mod tests {
    ...
    fn test_tt_store_and_probe() {
        ...
        tt.store(12345, 5, 100, NodeType::Exact, mv);
    ...
    fn test_tt_generation() {
        ...
        tt.new_generation();
        tt.store(12345, 5, 200, NodeType::Exact, mv);
    ...
    fn test_tt_thread_safety() {
        ...
        let handles: Vec<_> = (0..4).map(|t| {
            ...
            thread::spawn(move || {
                ...
                for i in 0..1000u64 {
                    let hash = t * 100000 + i;
                    tt.store(hash, 5, i as i32, NodeType::Exact, mv);
                }
            })
        }).collect();
