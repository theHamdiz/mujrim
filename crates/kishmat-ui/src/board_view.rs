...
use iced::widget::{button, column, container, mouse_area, row, text, Image};
...
impl BoardTheme {
    pub const ALL: [BoardTheme; 8] = [
        BoardTheme::Classic, BoardTheme::Emerald, BoardTheme::Ocean,
        BoardTheme::Royal, BoardTheme::Walnut,
        BoardTheme::Midnight, BoardTheme::Forest, BoardTheme::Sakura,
    ];
...
pub fn view_board<'a>(
    ...
    let premove_sqs: Vec<types::Square> = gs.premove_queue.iter()
    ...
    for display_row in 0..8 {
        ...
        for display_col in 0..8 {
            ...
            let bg_color = if let Some(flash) = capture_flash {
                ...
                if is_light { colors.legal_light } else { colors.legal_dark }
            } else if is_last_move {
                if is_light { colors.last_light } else { colors.last_dark }
            } else if is_light {
                colors.light
            } else {
                colors.dark
            ...
            let cell_content: Element<'a, Msg> = if let Some((fade_piece, fade_color, progress)) = fading_capture {
                match capture_anim_style {
                    ...
                    CaptureAnimStyle::Explosion => {
                        ...
                        let base_sz = if fade_piece == types::Piece::Pawn { piece_sz_pawn } else { piece_sz_normal };
                        ...
                        container(
                            Image::new(handle.clone()).width(piece_sz).height(piece_sz).opacity(opacity),
                        )
                        .center_x(sq_size)
                        .center_y(sq_size)
                        .into()
                ...
                if let Some((piece, color)) = gs.board.piece_on(sq) {
                    ...
                    container(
                        Image::new(handle.clone()).width(piece_sz).height(piece_sz),
                    )
                    .center_x(sq_size)
                    .center_y(sq_size)
                    .into()
                    ...
                    container(
                        container(text(""))
                            ...
                            .style(move |_theme| container::Style {
                                ...
                                border: iced::Border { radius: dot_br, ..Default::default() },
                                ..Default::default()
                            }),
                    )
                    .center_x(sq_size)
            ...
            let cell_with_coords: Element<'a, Msg> = if has_rank_label || has_file_label {
                ...
                if has_rank_label {
                    overlay_col = overlay_col.push(
                        text(format!(" {}", rank + 1)).size(10).color(coord_color),
                    );
                }
                ...
                overlay_col = overlay_col.push(
                    container(cell_content)
                        .center_x(sq_size).center_y(remaining)
                        .width(sq_size).height(remaining),
                );

                if has_file_label {
                    overlay_col = overlay_col.push(
                        container(
                            text(format!("{} ", files[display_col])).size(10).color(coord_color),
                        )
                        .width(sq_size)
            ...
            let cell = button(
                container(cell_with_coords).width(sq_size).height(sq_size),
            )
            .on_press(Msg::BoardClick(display_row, display_col))
            .width(sq_size)
            .height(sq_size)
            .style(move |_theme, status| {
                let hover_overlay = if matches!(status, button::Status::Hovered) { 0.04 } else { 0.0 };
                button::Style {
    ...
    let board_elem: Element<'a, Msg> = container(board_col)
        .style(move |_theme| container::Style {
            ...
            border: iced::Border { radius: 4.0.into(), width: 3.0, color: frame_color },
            ..Default::default()
        })
        .into();
    ...
    if show_coords && coord_position == CoordPosition::Outside {
        ...
        for display_row in 0..8 {
            ...
            rank_labels = rank_labels.push(
                container(text(format!("{}", rank + 1)).size(11).color(coord_label_color))
                    .height(sq_size).center_y(sq_size).width(20)
                    .align_x(Alignment::End)
            );
        }
        ...
        for display_col in 0..8 {
            file_labels = file_labels.push(
                container(text(format!("{}", files[display_col])).size(11).color(coord_label_color))
                    .width(sq_size).center_x(sq_size).height(20)
            );
        }

        // Assemble: rank labels | board, file labels below
        column![
            row![rank_labels, board_elem].spacing(0).align_y(Alignment::Start),
            file_labels,
        ].spacing(0).into()
    } else {
