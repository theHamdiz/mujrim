...
/// Default arrow fill color (chess.com orange, semi-transparent).
...
const ARROW_OUTLINE: Color = Color {
    r: 0.700,
    g: 0.350,
    b: 0.060,
    a: 0.90,
...
fn draw_straight_arrow(frame: &mut Frame, from: Point, to: Point, sq_size: f32) {
    let shaft_w = sq_size * 0.25;
...
fn draw_knight_arrow(
    ...
    draw_leg_segment(frame, from, corner, shaft_w);
...
fn draw_leg_segment(frame: &mut Frame, from: Point, to: Point, shaft_w: f32) {
    let dx = to.x - from.x;
    let dy = to.y - from.y;
    let len = (dx * dx + dy * dy).sqrt();
    if len < 1.0 {
