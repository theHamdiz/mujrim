...
type MenuInteractionQuery<'w, 's> = Query<
    'w,
    's,
...
type EngineTextQuery<'w, 's> = Query<
    'w,
    's,
    &'static mut Text,
    (
...
pub fn menu_button_system(
    mut interaction_q: MenuInteractionQuery,
...
pub fn update_hud(
    ...
    mut turn_text: TurnTextQuery,
    mut engine_text: EngineTextQuery,
    ...
    for mut text in turn_text.iter_mut() {
        ...
        let check = if game.board.in_check() {
            " (check)"
        } else {
            ""
        };
...
pub fn update_depth_text(
    ...
    let engine_name = config
        .selected_bundled_engine()
        .map(|engine| engine.display_name)
        .unwrap_or("Mujrim built-in");
    for mut text in text_q.iter_mut() {
        **text = format!("{engine_name} · depth {}", config.depth);
    }
}
