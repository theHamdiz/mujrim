...
type CoordQuery<'w, 's> = Query<
    'w,
    's,
    (&'static CoordLabel, &'static mut Transform),
...
pub fn resize_board(
    ...
    mut coords: CoordQuery<'_, '_>,
) {
    if !layout.is_changed() {
        return;
    }

    let flipped = game.is_some_and(|g| g.flipped);
...
pub fn handle_square_click(
    ...
    if game.selected_square.is_some() && game.legal_moves.iter().any(|m| m.to == sq) {
