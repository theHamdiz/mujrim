...
/// Bridge between mujrim-types Board and Bevy ECS.
#[derive(Resource)]
...
pub struct EngineConfig {
    ...
    /// Architecture-compatible bundled engines, strongest-first.
    pub bundled_engines: Vec<mujrim_protocols::catalog::DiscoveredEngine>,
    /// Index into `bundled_engines`; `None` selects the built-in engine.
    pub selected_engine: Option<usize>,
}
    ...
    fn default() -> Self {
        let bundled_engines =
            mujrim_protocols::catalog::discover_bundled_engines_from_environment()
                .unwrap_or_default();
        let selected_engine = (!bundled_engines.is_empty()).then_some(0);
        Self {
            depth: 6,
            time_limit_ms: 2000,
            bundled_engines,
...
impl EngineConfig {
    pub fn selected_bundled_engine(&self) -> Option<&mujrim_protocols::catalog::DiscoveredEngine> {
        self.selected_engine
            .and_then(|index| self.bundled_engines.get(index))
    }
}
...
mod tests {
    ...
    #[test]
    fn engine_selection_is_bounds_checked() {
        let mut config = EngineConfig {
            depth: 8,
            time_limit_ms: 500,
            bundled_engines: vec![mujrim_protocols::catalog::DiscoveredEngine {
                id: "stockfish",
                display_name: "Stockfish",
                path: std::path::PathBuf::from("stockfish.exe"),
                target_directory: "windows-aarch64".to_owned(),
                compatibility: mujrim_protocols::catalog::RuntimeCompatibility::Native,
                search_limits: mujrim_protocols::catalog::SearchLimitSupport::STANDARD,
            }],
            selected_engine: Some(0),
        };
        assert_eq!(
            config.selected_bundled_engine().map(|engine| engine.id),
            Some("stockfish")
        );
        config.selected_engine = Some(1);
        assert!(config.selected_bundled_engine().is_none());
    }
}
