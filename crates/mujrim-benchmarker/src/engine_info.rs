...
pub struct NnueInfo {
    ...
    pub search_profile: eval::nnue::NnueSearchProfile,
}

impl NnueInfo {
    /// Detect the current embedded NNUE network info.
    pub fn detect() -> Self {
        use eval::nnue::{NnueNetworkSource, default_embedded_network};

        let network = default_embedded_network();
        let profile = network.search_profile();
        Self::from_runtime_with_profile(network.info(), profile)
    }
    ...
    pub fn from_runtime(info: eval::nnue::NnueNetworkInfo) -> Self {
        ...
        Self::from_runtime_with_profile(info, profile)
    }

    fn from_runtime_with_profile(
        info: eval::nnue::NnueNetworkInfo,
        search_profile: eval::nnue::NnueSearchProfile,
    ) -> Self {
    ...
    pub fn display_lines(&self) -> Vec<String> {
        vec![
            ...
            format!("    Search:     {} profile", self.search_profile.as_str()),
        ]
    }
}
...
pub fn detect_techniques(params: &SearchParams) -> Vec<Technique> {
    vec![
        ...
        Technique {
            ...
            enabled: true,
            details: "quadratic depth and move-history threshold".into(),
        },
        Technique {
...
mod tests {
    ...
    fn test_nnue_info_detect() {
        ...
        let runtime = default_embedded_network().info();
        assert!(info.hidden_size > 0);
        assert!(info.num_buckets > 0);
        assert_eq!(info.name, runtime.name);
        assert_eq!(info.format, runtime.format.to_string());
        assert_eq!(info.architecture, runtime.architecture);
        assert_eq!(
            info.search_profile,
            default_embedded_network().search_profile()
        );
    }
