...
//! `cargo test -p kishmat-search --release --test bk_tactical_targets bk_all_tactical_misses_sequential -- --ignored --nocapture --test-threads=1`
//!
//! Per-case tests run in parallel by default; use `--test-threads=1` if you run
//! `cargo test ... -- --ignored` so each case gets full cores.
//!
//! FENs stay in sync with `kishmat-benchmarker::suite::BK_POSITIONS` indices
//! **4, 6, 8, 9, 11, 14, 16, 17, 22** (zero-based).

use kishmat_search::SearchEngine;
...
const BK_TACTICAL_MISSES: &[(&str, &str)] = &[
    ...
    (
        "r1bq1rk1/pp2ppbp/2np2p1/2n5/P3PP2/3pBN2/1PP1BPPP/RN1Q1RK1 b - - 0 1",
        "c5b3",
    ),
    (
...
#[test]
#[ignore = "Aspirational: engine should find a2a4 at high depth"]
fn bk_miss_01_a2a4() {
    setup();
