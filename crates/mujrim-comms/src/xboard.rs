...
mod tests {
    ...
    #[test]
    fn xboard_uses_conservative_startup_resources() {
        let handler = XBoardHandler::new();
        assert_eq!(handler.hash_mb, DEFAULT_HASH_MB);
        assert_eq!(handler.threads, DEFAULT_THREADS);
        assert_eq!(handler.engine.num_threads, DEFAULT_THREADS);
    }
