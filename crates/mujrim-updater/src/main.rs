...
//! Mujrim Updater — pulls updates from GitHub releases for all components.
//!
//! Commands:
//! - `check`          — Check for available updates
//! - `update all`     — Update all components
//! - `update <crate>` — Update a specific component (e.g., `mujrim-ui`)
...
const COMPONENTS: &[&str] = &[
    "mujrim",         // Main engine binary (UCI)
    "mujrim-ui",      // GUI application
    "mujrim-updater", // This updater itself
];
...
fn main() {
    let matches = Command::new("Mujrim Updater")
        .version(VERSION)
        .author("Ahmad Hamdi Emara")
        .about("Updates Mujrim chess engine components from GitHub releases")
            ...
            Command::new("syzygy")
                ...
                .arg(
                    Arg::new("pieces")
                        .short('p')
                        .long("pieces")
                        .help("Max pieces: 5 (~939 MiB), 6 (~149 GiB), 7 (~16.7 TiB)")
                        .default_value("5")
                        .value_parser(["5", "6", "7"]),
                )
                .arg(
                    Arg::new("dir")
                        ...
                        .default_value(mujrim_updater::syzygy::DEFAULT_SYZYGY_DIR),
                ),
        )
        .subcommand(
            Command::new("nnue")
                ...
                .arg(
                    Arg::new("dir")
                        ...
                        .default_value(mujrim_updater::nnue::DEFAULT_NNUE_DIR),
                ),
        )
        .get_matches();
...
fn cmd_nnue(network: &str, dir: &str) {
    ...
    if network == "list" {
        ...
        println!();
        println!("  To download all: mujrim-updater nnue");
        println!("  To download one: mujrim-updater nnue -n \"Akimbo 1024\"");
        return;
    }
    ...
    if network == "all" {
        ...
        match net {
            ...
            None => {
                ...
                eprintln!("  Use 'mujrim-updater nnue -n list' to see available networks.");
            }
        }
    }
}
...
fn cmd_check() {
    println!("╔══════════════════════════════════════╗");
    println!("║     Mujrim Update Checker v{VERSION}    ║");
    println!("╚══════════════════════════════════════╝");
    println!();
    ...
    match fetch_latest_release() {
        Ok(release) => {
            ...
            if release.tag_name.contains(VERSION) {
                ...
                println!("  ⚡ Update available! Run: mujrim-updater update all");
            }
        }
        Err(e) => {
...
fn cmd_update(component: &str) {
    println!("╔══════════════════════════════════════╗");
    println!("║       Mujrim Updater v{VERSION}         ║");
    println!("╚══════════════════════════════════════╝");
    println!();
...
fn cmd_list() {
    println!("╔══════════════════════════════════════╗");
    println!("║     Mujrim Components v{VERSION}        ║");
    println!("╚══════════════════════════════════════╝");
    println!();
...
fn fetch_latest_release() -> Result<Release, String> {
    ...
    let client = reqwest::blocking::Client::builder()
        .user_agent(format!("mujrim-updater/{VERSION}"))
        .build()
...
fn update_component(release: &Release, component: &str, install_dir: &Path) {
    let platform = current_platform_tag();
...
fn download_and_extract(asset: &Asset, install_dir: &Path) -> Result<usize, String> {
...
fn extract_tar_gz(archive: &Path, dest: &Path) -> Result<usize, String> {
...
fn get_install_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    {
        if let Ok(local_app_data) = std::env::var("LOCALAPPDATA") {
            let dir = PathBuf::from(local_app_data).join("Mujrim").join("bin");
