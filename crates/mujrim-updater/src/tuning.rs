...
impl TunableParams {
    ...
    pub fn set_value(&mut self, section: &str, name: &str, value: f64) -> bool {
        let param = if let Some(group_name) = section.strip_prefix("search.") {
            self.search
                .get_mut(group_name)
                .and_then(|group| group.get_mut(name))
        } else {
            match section {
                "history" => self.history.get_mut(name),
                "correction" => self.correction.get_mut(name),
                "time" => self.time.get_mut(name),
                _ => None,
            }
        };
...
mod tests {
    ...
    fn test_set_value() {
        ...
        assert!(params.set_value("search.test_group", "test_param", 150.0));
        assert!(!params.set_value("search.missing", "test_param", 50.0));
        assert!(!params.set_value("unknown", "test_param", 50.0));
        let flat = params.flat_list();
        let found = flat.iter().find(|(_, n, _)| n == "test_param").unwrap();
        assert_eq!(found.2.value, 100.0);
    }
}
