...
//!
//! Wraps `kishmat-updater` download functions for use in the async iced UI.
...
/// All available tiers for the UI picker.
pub fn syzygy_tiers() -> Vec<(SyzygyPieceSet, &'static str)> {
    vec![
        (SyzygyPieceSet::Standard, "3-4-5 pieces (~1 GB)"),
        (SyzygyPieceSet::Extended, "3-4-5-6 pieces (~150 GB)"),
        (SyzygyPieceSet::Full, "3-4-5-6-7 pieces (~140 TB)"),
    ]
}
...
mod tests {
    ...
    #[test]
    fn syzygy_tiers_non_empty() {
        assert_eq!(syzygy_tiers().len(), 3);
    }

    #[test]
    fn nnue_dir_is_subdir() {
        let dir = nnue_dir(Path::new("/opt/kishmat"));
        assert_eq!(dir, PathBuf::from("/opt/kishmat/nnue"));
    }

    #[test]
    fn syzygy_dir_is_subdir() {
        let dir = syzygy_dir(Path::new("/opt/kishmat"));
        assert_eq!(dir, PathBuf::from("/opt/kishmat/syzygy"));
    }
}
