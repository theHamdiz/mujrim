...
pub fn default_install_dir() -> PathBuf {
    match std::env::consts::OS {
        ...
        "windows" => {
            ...
            PathBuf::from(local).join("KishMat").join("bin")
        }
        _ => PathBuf::from("."),
    }
}
...
fn create_macos_bundles(bin_dir: &Path) -> Result<usize, String> {
    ...
    for bin in embedded::BINARIES.iter().filter(|b| b.create_shortcut) {
        let app_name = match bin.filename {
            "kishmat-ui" => "KishMat Chess",
            "kishmat-game" => "KishMat Game",
            _ => bin.name,
        };
        ...
        // Write Info.plist
        let bundle_id = format!("com.kishmat.{}", bin.filename.replace('-', ""));
        let plist = format!(
...
fn create_linux_desktop_entries(bin_dir: &Path) -> Result<usize, String> {
    ...
    for bin in embedded::BINARIES.iter().filter(|b| b.create_shortcut) {
        ...
        let desktop = format!(
            ...
            exec = bin_dir.join(bin.filename).display(),
...
fn create_windows_shortcuts(bin_dir: &Path) -> Result<usize, String> {
    ...
    let _logo_png = include_bytes!("../assets/logo.png");
    let icon_dir = bin_dir.parent().unwrap_or(bin_dir);
    let icon_path = icon_dir.join("kishmat.ico");
...
mod tests {
    ...
    fn desktop_entry_format() {
        let entry = desktop_entry("KishMat", "/usr/bin/kishmat", "/icon.png", "Chess");
        assert!(entry.contains("Name=KishMat"));
        assert!(entry.contains("Exec=/usr/bin/kishmat"));
    ...
    fn install_dir_platform_specific() {
        ...
        if cfg!(target_os = "windows") {
            assert!(dir.to_string_lossy().contains("KishMat"));
        } else {
