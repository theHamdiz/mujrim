...
impl ThreadPool {
    ...
    pub fn configured_thread_limit() -> usize {
        const MAXIMUM_THREADS: usize = 256;
        std::thread::available_parallelism().map_or(1, std::num::NonZeroUsize::get).min(MAXIMUM_THREADS)
    }
    ...
    pub fn set_count(&mut self, threads: usize) {
        let threads = threads.clamp(1, Self::configured_thread_limit());
    ...
    pub fn execute_searches(
        &mut self, time_manager: TimeManager, report: Report, multi_pv: usize, restricted_root_moves: Option<&[Move]>,
        board: &Board, shared: &Arc<SharedContext>,
    ) {
        ...
        #[cfg(target_arch = "wasm32")]
        {
...

            {
                ...
                t1.root_moves = root_moves_for(&t1.board, restricted_root_moves);
...
fn root_moves_for(board: &Board, restricted: Option<&[Move]>) -> Vec<RootMove> {
    restricted.map_or_else(
        || board.generate_all_moves().iter().map(|entry| RootMove { mv: entry.mv, ..Default::default() }).collect(),
        |moves| moves.iter().copied().map(|mv| RootMove { mv, ..Default::default() }).collect(),
    )
}
...
mod tests {
    ...
    #[test]
    fn configured_threads_follow_available_hardware_without_oversubscription() {
        let available = std::thread::available_parallelism().map_or(1, std::num::NonZeroUsize::get);
        assert_eq!(ThreadPool::configured_thread_limit(), available.min(256));
        assert!(ThreadPool::configured_thread_limit() >= 1);
        assert!(ThreadPool::available_threads() >= ThreadPool::configured_thread_limit());
    }
}
