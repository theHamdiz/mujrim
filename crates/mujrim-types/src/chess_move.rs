...
impl Move {
    ...
    /// Is this a non-capture, non-promotion move?
    #[inline(always)]
...
impl MoveList {
    ...
    /// Shortens the list, retaining the first `len` moves.
    #[inline(always)]
...
mod tests {
    ...
    fn test_move_list_push_and_iter() {
        ...
        ml.truncate(1);
        assert_eq!(ml.as_slice(), &[Move::quiet(Square::E2, Square::E4)]);
    }

    #[test]
    fn move_list_clone_swap_and_sort_preserve_initialized_prefix() {
        let mut moves = MoveList::new();
        moves.push(Move::quiet(Square::G1, Square::F3));
        moves.push(Move::quiet(Square::E2, Square::E4));
        moves.push(Move::quiet(Square::D2, Square::D4));

        moves.swap(0, 2);
        moves.sort_by(|left, right| left.to_uci().cmp(&right.to_uci()));

        let cloned = moves.clone();
        let uci: Vec<_> = cloned.iter().map(|mv| mv.to_uci()).collect();
        assert_eq!(uci, ["d2d4", "e2e4", "g1f3"]);
    }
}
