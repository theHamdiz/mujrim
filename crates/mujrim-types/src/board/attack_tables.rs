...
fn boxed_zeroed<T>() -> Box<T> {
    ...
    // SAFETY: Attack lookup storage contains integer arrays, for which zero is valid.
    let pointer = unsafe { std::alloc::alloc_zeroed(layout).cast::<T>() };
    if pointer.is_null() {
        std::alloc::handle_alloc_error(layout);
    }
    // SAFETY: `pointer` came from the global allocator with the exact layout for `T`.
...
fn rook_mask(sq: usize) -> Bitboard {
    let (r, f) = (sq / 8, sq % 8);
    let mut mask = 0u64;
    for cr in (r as i32 + 1)..7 {
        mask |= 1u64 << (cr * 8 + f as i32);
    }
    for cr in (1..r as i32).rev() {
        mask |= 1u64 << (cr * 8 + f as i32);
    }
    for cf in (f as i32 + 1)..7 {
        mask |= 1u64 << (r as i32 * 8 + cf);
    }
    for cf in (1..f as i32).rev() {
        mask |= 1u64 << (r as i32 * 8 + cf);
    }
    mask
}
