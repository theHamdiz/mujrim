//! Piece-Square Tables (PeSTO-based values for middlegame and endgame).
//! These tables encode positional bonuses/penalties for each piece on each square.

use types::Piece;

// PeSTO PSQT values are indexed by the square from White's perspective.
// Black's squares are mirrored vertically.
...
pub fn mg_value(piece: Piece, sq_index: usize) -> i32 {
    match piece {
        Piece::Pawn => PAWN_MG[sq_index],
        Piece::Knight => KNIGHT_MG[sq_index],
        Piece::Bishop => BISHOP_MG[sq_index],
        Piece::Rook => ROOK_MG[sq_index],
...
pub fn eg_value(piece: Piece, sq_index: usize) -> i32 {
    match piece {
        Piece::Pawn => PAWN_EG[sq_index],
        Piece::Knight => KNIGHT_EG[sq_index],
        Piece::Bishop => BISHOP_EG[sq_index],
        Piece::Rook => ROOK_EG[sq_index],
        Piece::Queen => QUEEN_EG[sq_index],
        Piece::King => KING_EG[sq_index],
    }
}
...
mod tests {
    ...
    #[test]
    fn test_psqt_pawn_values() {
        // Center pawns should have positive middlegame values
        assert!(PAWN_MG[Square::D4.index()] > 0);
        assert!(PAWN_MG[Square::E4.index()] > 0);
    }

    #[test]
    fn test_king_safety_values() {
        // Castled king position (G1) should have decent middlegame value
        // King in center (E1) should have lower middlegame value
        let g1_val = KING_MG[Square::G1.index()];
