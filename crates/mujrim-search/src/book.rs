...
impl OpeningBook {
    ...
    /// Loads a supported opening-book file into memory.
    pub fn load(path: &Path) -> Result<Self, String> {
        ...
        if !extension.eq_ignore_ascii_case("bin") {
            return Err(format!(
                "unsupported opening-book format '{}'; supported: Polyglot .bin",
                path.display()
            ));
        }
        let data = std::fs::read(path).map_err(|error| {
            format!("failed to read opening book '{}': {error}", path.display())
        })?;
        if data.is_empty() {
            return Err(format!("opening book '{}' is empty", path.display()));
        }
        Self::from_bytes(&data)
    }

    /// Discovers the pinned book by fingerprint, independent of filename.
    pub fn load_default() -> Result<Self, String> {
        const PINNED_SHA256: &str =
            "d819575232ac8acb0ea7edc3df31490bd436cd89c6cbef142f0d008cbc5a8662";
        let mut candidates = book_candidates();
        candidates.sort();
        candidates.dedup();
        for path in &candidates {
            if sha256_file(path).as_deref() == Some(PINNED_SHA256) {
                return Self::load(path);
            }
        }
        Err(format!(
            "pinned opening book SHA-256 {PINNED_SHA256} was not found in a books/ directory"
        ))
    }

    /// Compatibility alias retained for callers while releases transition to
...
fn book_candidates() -> Vec<PathBuf> {
    let mut candidates = Vec::new();
    if let Some(explicit) = std::env::var_os("MUJRIM_BOOK") {
        let path = PathBuf::from(explicit);
        if path.is_file() {
            candidates.push(path);
        } else {
            collect_book_files(&path, 0, &mut candidates);
        }
    }
    if let Ok(executable) = std::env::current_exe() {
        for ancestor in executable
            ...
            .take(7)
        {
            collect_book_files(&ancestor.join("books"), 0, &mut candidates);
        }
    }
    if let Ok(current) = std::env::current_dir() {
        collect_book_files(&current.join("books"), 0, &mut candidates);
        collect_book_files(&current.join("dist").join("books"), 0, &mut candidates);
    }
    candidates
}

fn collect_book_files(directory: &Path, depth: usize, output: &mut Vec<PathBuf>) {
    if depth > 2 {
        return;
    }
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            collect_book_files(&path, depth + 1, output);
        } else if path
            .extension()
