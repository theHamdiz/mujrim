...
impl fmt::Display for SystemInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "Mujrim GPU/NPU Detection")?;
...
fn windows_total_memory_bytes() -> Option<u64> {
    ...
    // SAFETY: `status` has the layout required by MEMORYSTATUSEX, its length
    // field is initialized, and the mutable pointer remains valid for the call.
    let succeeded = unsafe { GlobalMemoryStatusEx(&mut status) } != 0;
    succeeded.then_some(status.total_phys)
}
...
mod tests {
    ...
    #[test]
    fn memory_bytes_are_reported_as_mebibytes() {
        assert_eq!(bytes_to_mebibytes(16 * 1024 * 1024 * 1024), 16 * 1024);
        assert_eq!(bytes_to_mebibytes(1024 * 1024 - 1), 0);
    }
}
