...
mod tests {
    ...
    fn test_castling_generation() {
        ...
        assert_eq!(castling_moves.len(), 2, "Should have 2 castling moves");
    }
    ...
    fn test_no_castling_through_check() {
        ...
        assert!(kcastle.is_empty(), "Should not castle through check on f1");
    }
    ...
    fn test_no_castling_while_in_check() {
        ...
        assert!(board.in_check());
        let moves = board.generate_legal_moves();
        let castling: Vec<_> = moves.iter().filter(|m| m.is_castling()).collect();
        assert!(castling.is_empty(), "Should not castle while in check");
    }
    ...
    fn test_promotion_generates_four_options() {
        ...
        assert_eq!(promos.len(), 4, "Should have 4 promotion options (Q/R/B/N)");
    }
