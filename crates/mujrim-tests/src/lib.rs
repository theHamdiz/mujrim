//! Integration tests for Mujrim chess engine.
...
mod integration {
    ...
    fn test_uci_handles_garbage_input() {
        ...
        // Malformed commands must not corrupt the current board.
        assert_eq!(handler.board.piece_count(Piece::King, Color::White), 1);
        assert_eq!(handler.board.piece_count(Piece::King, Color::Black), 1);
    }
