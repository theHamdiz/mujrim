...
pub struct BoardViewOptions<'a> {
    pub sq_size: f32,
    pub anim: Option<AnimInfo>,
...
fn piece_view<'a>(
    ...
    Svg::new(assets.get(piece_set, piece, color).clone())
        .width(size)
        .height(size)
        .opacity(opacity)
        .into()
...
pub fn view_board<'a>(
    gs: &'a GameState,
    assets: &'a PieceAssets,
    options: BoardViewOptions<'a>,
) -> Element<'a, Msg> {
    let BoardViewOptions {
        sq_size,
        anim,
        theme,
        show_coords,
    ...
    for display_row in 0..8 {
        ...
        for (display_col, file_label) in files.iter().copied().enumerate() {
                    ...
                    match capture_anim_style {
                        ...
                        CaptureAnimStyle::Explosion => {
                            ...
                            container(piece_view(
                                assets, piece_set, fade_piece, fade_color, piece_sz, opacity,
                            ))
                            .center_x(sq_size)
                            .center_y(sq_size)
                            .into()
            ...
            let cell_with_coords: Element<'a, Msg> = if has_rank_label || has_file_label {
                ...
                if has_file_label {
                    overlay_col = overlay_col.push(
                        container(text(format!("{file_label} ")).size(10).color(coord_color))
                            .width(sq_size)
