...
- GUI tournaments handshake each engine's advertised UCI options and only set Hash/Threads/EvalFile/WeightsFile when that engine implements them. Official `lc0` uses `plan_launch`. There is no `mujrim-lc0` product binary. Event Elo is a field rating (not CCRL 40/15); Stockfish is the only hard CCRL pin.
